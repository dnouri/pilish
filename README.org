...
Press =w= on one strict file target at point to copy the same shell-local
path that =!= operates on.  Local paths are absolute; paths in remote sessions
use the remote shell's namespace without an Emacs TRAMP prefix.  Line, column,
range, and link-fragment metadata are excluded.  The kill-ring value is the
literal, unquoted pathname, so spaces remain spaces and no =@= prompt marker is
added.
