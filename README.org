...
ancestor from the old snapshot, then the new active path.  Switching
the linked chat to another session file resets this orientation even
when a fork shares node IDs with the old file.

Press =RET= to continue from the selected turn.  Selecting the =@=
entry is a strict no-op before live-process and busy/transition checks,
including offline or streaming sessions and a current user prompt
followed only by projected-away bookkeeping records; it neither reads
or rewrites the session nor replaces the input draft.  A historical
user prompt or =custom_message= is user-like: it continues from its
parent and prefills its text when available for editing.  If that
parent does not exist, Pilish points you to the chat's fork command.
An assistant or any other historical entry continues from itself.
...
cannot be labeled or used as a continuation target.

=f= cycles filters
(starting from the =no-tools= default; see
=pilish-tree-browser-default-filter=), =/= searches, and =l= sets or
clears a label on a node.  Press =?= for its menu and =g= to refresh
the last persisted state.  Display and refresh read the JSONL file from
disk and still work offline; changing position by continuing from a
historical turn requires the linked live, idle Pilish process.
...
Magit's own =j= / =k= section navigation works unmodified.  In the
session browser =RET= switches to the selected session; in the tree
browser it continues from the selected turn.  Every documented browser
key — views, filters, search, scope, rename, delete, refresh, dispatch,
=RET= — is rebound in motion state so the Evil and evil-collection
