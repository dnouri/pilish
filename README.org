...
after confirmation, using Emacs's trash when configured.
...
conversation to the node at point, =f= cycles filters (starting from
the =no-tools= default; see =pilish-tree-browser-default-filter=),
=/= searches, and =l= sets or clears a label on a node.  Press =?=
for its menu and =g= to refresh the last persisted state.
...
session file; =RET= picks which branch the live conversation
continues on.  =C-c C-p f= (=/fork=, or Pi's =/clone=) instead writes
...
documented browser key — views, filters, search, scope, rename,
delete, refresh, dispatch, =RET= — is rebound in motion state so the
Evil and evil-collection keymap stack never swallows them (evil's
